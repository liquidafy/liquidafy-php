 ...
 *
 * Mirrors the backend's signing scheme byte-for-byte
 * (liquidafy-backend `internal/module/webhook/domain/sign.go`):
 *
...
final class Webhook
{
    ...
    /**
     * Verification core — throws on any failure. Mirrors the backend's
     * `VerifySignature` exactly:
    ...
    /**
     * `hex(hmac-sha256(secret, "<timestamp>.<payload>"))` — identical
     * to the backend's `SignPayload`. Useful for building test
    ...
    /**
     * Parses `t=<unix>,v1=<hex>` — same semantics as the backend
     * parser: every comma-separated part must be `k=v`; unknown keys
     * are ignored; `t` must be ASCII digits only; on duplicates the
     * last value wins; both `t` and `v1` are required.
     *
